<x-layout :title="$pageTitle">
    ...
    <div class="mt-6 flex items-center justify-end gap-x-6">
        <a href="/blog/create"
            class="rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-gray-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">Create</a>
        </div>
  @foreach ($posts as $post)
    <div class="flex justify-between items-center border border-gray-200 px-4 py-6 my-2">
        ...
        <div>
            ...
            <form method="POST" action="/blog/{{ $post->id }}"class="inline" onsubmit="return confirm('Are you sure?');">
                @csrf
                @method('DELETE')
