...
<form method="POST" action="/blog/{{ $post->id }}" class="mt-6 space-y-8">
    @csrf
    @method('PATCH')
  ...
  <input type="hidden" name="id" value="{{ $post->id }}">
    ...
    <div class="border-b border-white/10 pb-12">
      ...
      <p class="mt-1 text-sm/6 text-gray-400">Use This Form To update post data to the blog</p>
     
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
     <div class="sm:col-span-3">
  <label for="title" class="block text-sm/6 font-medium text-gray-900">Title</label>
  <div class="mt-2">
    <input id="title" type="text" name="title" value="{{old('title' , $post->title )}}" autocomplete="family-name"
...
<div class="col-span-full">
  ...
  <div class="mt-2">
    <textarea id="body" name="body" rows="3"
      class="block w-full rounded-md border {{ $errors->has('body') ? 'border-red-500 focus:border-red-500 focus:ring-red-200' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }} px-3 py-1.5 text-base text-gray-900 placeholder:text-gray-500 sm:text-sm">
         ...
         <div class="flex gap-3">
              <div class="flex h-6 shrink-0 items-center">
                <div class="group grid size-4 grid-cols-1">
                  <input id="published" type="checkbox" name="published" checked aria-describedby="published-description" value="1"
                   {{old('published') || (!old() && $post->published) ? 'checked' : '' }}
                  class="col-start-1 row-start-1 appearance-none rounded-sm border border-gray-300 bg-white checked:border-indigo-500 checked:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500" />
